...
def get_datasets(freq, time_step=60):
    ...
    # --- CORRECTED SCALING AND SPLITTING LOGIC (Prevents Data Leakage) ---
    # 1. Split the unscaled data first
    ...
    # 3. Transform all three sets using the fitted scaler
    ...
    # Check for empty sets after sequence creation
...
class MOGWO:
    def __init__(self, problem_func, bounds, pack_size=5, max_iter=5):
        self.problem_func = problem_func
        self.bounds = bounds
        self.pack_size = pack_size
        self.max_iter = max_iter
        self.wolves = [GreyWolf(bounds) for _ in range(pack_size)]
        self.archive = []
    ...
    def _update_archive(self):
        combined = self.archive + self.wolves
        non_dominated = []
        for wolf in combined:
            is_dominated = False
            for other in combined:
                if self._dominates(other.score, wolf.score):
                    is_dominated = True
                    break
            if not is_dominated:
                non_dominated.append(wolf)
        self.archive = sorted(non_dominated, key=lambda w: w.score[0])[:100]

    def optimize(self):
        for t in range(self.max_iter):
            print(f"MOGWO Iteration {t+1}/{self.max_iter}")
            for wolf in self.wolves:
                wolf.score = self.problem_func(wolf.position)

            self._update_archive()
            if not self.archive: continue

            best_rmse_in_iter = min(w.score[0] for w in self.archive)
            ...
            leaders = np.random.choice(self.archive, 3, replace=True)
            alpha, beta, delta = leaders[0], leaders[1], leaders[2]
            ...
            for wolf in self.wolves:
                ...
                wolf.position = np.clip(wolf.position, self.bounds[:, 0], self.bounds[:, 1])
...
def plot_results(pareto_solutions, convergence, train_hist, val_hist, base_save_dir, exp_name):
    ...
    plt.title(f'MOGWO Convergence for {exp_name}')
    plt.grid(True)
...
if __name__ == "__main__":
    ...
    for freq_name, freq_code in FREQUENCIES_TO_RUN.items():
        ...
        # --- MODIFICATION: Dynamically set time_step to prevent errors ---
        if freq_code == 'W':
            time_step_for_run = 8  # Use 8 weeks of history for weekly data
        elif freq_code == 'D':
            time_step_for_run = 30 # Use 30 days of history for daily data
        else:
            time_step_for_run = 60 # Use 60 steps for minutely and hourly data
        ...
        mogwo = MOGWO(problem_func, BOUNDS, pack_size=5, max_iter=5)
        pareto_solutions, convergence_history = mogwo.optimize()
        
        if not pareto_solutions:
            print(f"!!! MOGWO found no solutions for {exp_name}. Skipping. !!!")
            continue
