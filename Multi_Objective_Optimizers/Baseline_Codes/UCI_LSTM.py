...
def perform_wilcoxon_test(y_true, y_pred_model, y_pred_baseline):
    ...
    try:
        ...
        # Happens if errors are identical
        p_value = 1.0
        significant = False
...
def train_model(model, train_loader, val_loader, epochs, lr, device, early_stop_patience=10):
    ...
    for epoch in range(epochs):
        ...
        if len(preds) > 0:
            ...
            val_hist.append(val_rmse**2) # Storing MSE for plot consistency
...
LSTM_GRID = [
    {"hidden_dim": 64, "layers": 1, "lr": 1e-3, "dropout": 0.0, "batch_size": 64},
    {"hidden_dim": 128, "layers": 2, "lr": 1e-3, "dropout": 0.2, "batch_size": 64},
]

def run_lstm_fair_comparison(train_ds, val_ds, test_ds, scaler, device, freq_name, save_dir, seq_len):
    ...
    for i, cfg in enumerate(LSTM_GRID):
        ...
        # Train (search phase uses 50 epochs)
        model, _, _, _, val_rmse = train_model(model, train_loader, val_loader, epochs=50, lr=cfg["lr"], device=device)
        
        print(f"[{i+1}/{len(LSTM_GRID)}] Config: {cfg} (Adj Batch: {bs}) -> Val RMSE: {val_rmse:.4f}")
    ...
    # Train (retrain phase uses 100 epochs)
    ...
    # Plotting
    plot_actual_vs_predicted(trues_scaled, preds_scaled, freq_name, save_dir, "LSTM")

    plt.figure()
    plt.plot(train_hist, label="Train Loss (Combined)")
    plt.title(f"LSTM Learning Curve - {freq_name}")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.savefig(os.path.join(save_dir, f"learning_curve_lstm_{freq_name}.png"))
    plt.close()
    ...
    print(f"\nMetrics on Test Set:")
    print(f"{'Metric':<10} | {'LSTM':<15} | {'Naive':<15}")
    print(f"{'-'*10}-|-{'-'*15}-|-{'-'*15}")
    ...
    print(f"Plot saved to {save_dir}/LSTM_{freq_name}_comparison.png")
