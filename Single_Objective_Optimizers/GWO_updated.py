...
def get_datasets_for_frequency(file_path, freq_code, time_step):
    ...
    n = len(df_resampled)
    if n < time_step * 2:
        print(f"Error: Not enough data for frequency {freq_code}. Need at least {time_step*2}, but got {n}.")
        return None, None, None, None
...
class GWO:
    def __init__(self, problem, bounds, wolf_count=5, max_iter=5):
        self.problem = problem
        self.bounds = np.array(bounds)
        self.wolf_count = wolf_count
        self.max_iter = max_iter
        self.dim = self.bounds.shape[0]
        self.positions = np.random.uniform(self.bounds[:, 0], self.bounds[:, 1], (wolf_count, self.dim))
    ...
    def optimize(self):
        # The computational budget is wolf_count * max_iter = 5 * 5 = 25 evaluations.
        # This is fair and matches the PSO reference script.
        for t in range(self.max_iter):
            print(f"Iteration {t+1}/{self.max_iter}")
            for i in range(self.wolf_count):
                fitness = self.problem(self.positions[i])
                ...
                elif self.beta_score < fitness < self.delta_score:
...
if __name__ == "__main__":
    ...
    for freq_name, freq_code in FREQUENCIES_TO_RUN.items():
        print(f"\n{'='*25} RUNNING GWO FOR {freq_name} FREQUENCY {'='*25}")
        ...
        gwo = GWO(evaluate_candidate, bounds, wolf_count=5, max_iter=5)
        best_position, best_rmse = gwo.optimize()
        ...
        save_dir = os.path.join('results_gwo_fair_50epochs', freq_name)
